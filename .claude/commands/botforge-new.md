...
1. **Business Brief** — задай до 5 уточняющих вопросов (purpose + основной путь пользователя / monetization / audience / integrations / hosting). Если всё указано в запросе — пропусти.
2. **ADR** — Architecture Decision Record в пределах 250 слов: стек + обоснование, модель данных, модульная раскладка, карта навигации, зависимости, деплой, риски, точки расширения.
...
Отдельно проверь UX-навигацию: нет мёртвых кнопок, у вложенных экранов есть back/home, у FSM есть cancel, callback-и отвечают через `call.answer()`, клавиатуры не превращаются в плотные сетки.
