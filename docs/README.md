# Documentation index

## Start here

- [QUICKSTART](QUICKSTART.md) — 5 minutes from zero to bot
- [INSTALL](INSTALL.md) — per-tool installation details

## Usage

- [USAGE](USAGE.md) — modes (Lite / Pro / Media / SaaS), prompt formats, session lifecycle
- [COMPARISON](COMPARISON.md) — BotForge vs plain prompts, cookiecutter, no-code platforms

## Maintenance

- [MIGRATION](MIGRATION.md) — upgrading between versions
- [CHANGELOG](CHANGELOG.md) — version history
- [SYNC-CHECKLIST](SYNC-CHECKLIST.md) — four-format skill-prompt sync (contributors)

## Community

- [SHOWCASE](SHOWCASE.md) — bots built with BotForge
- [CONTRIBUTING](../CONTRIBUTING.md) — how to add commands, references, examples
- [CODE_OF_CONDUCT](../CODE_OF_CONDUCT.md)
- [SECURITY](../SECURITY.md) — vulnerability disclosure

## Translations

- [Русский README](ru/README.md)
- [Polski README](pl/README.md)

## Reference library (inside skill bundle)

23 reference modules live in [`.claude/skills/botforge/references/`](../.claude/skills/botforge/references/):

- architecture.md · patterns.md · examples.md · checklists.md · anti-patterns.md
- miniapp.md · auth.md · payments.md · admin-panel.md
- telegram-api-spec.md · botfather-setup.md
- i18n.md · observability.md · analytics.md · performance.md
- scheduler.md · subscriptions.md · inline-mode.md
- groups-and-channels.md · media.md · anti-spam.md
- gdpr-compliance.md · faq.md
